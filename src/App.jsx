...
.header-shell {
  ...
  gap: 56px;
  background:
    radial-gradient(circle at 18% 10%, rgba(226, 171, 45, 0.09), transparent 11%),
    radial-gradient(circle at 54% 9%, rgba(18, 47, 95, 0.18), transparent 16%),
    var(--header);
}
...
.choose-card {
  ...
  box-shadow: 0 24px 56px rgba(18, 47, 95, 0.12);
}
...
.choose-list li {
  ...
  border-radius: 16px;
  background: #fff;
  box-shadow: 0 10px 24px rgba(18, 47, 95, 0.07);
  color: #262b35;
  font-weight: 600;
  text-align: left;
  white-space: normal;
...
.choose-list li::before {
  content: counter(choose-item, decimal-leading-zero);
  width: 42px;
  height: 42px;
  flex: 0 0 42px;
  display: grid;
  place-items: center;
  border-radius: 12px;
  background: rgba(226, 171, 45, 0.16);
  color: var(--gold-dark);
  font-size: 13px;
  font-weight: 800;
  line-height: 1;
...
.back-button {
  position: absolute;
  top: 82px;
  left: max(34px, calc((100% - 1100px) / 2));
  border: 0;
  background: transparent;
}
...
.input-wrap:focus-within {
  border-color: rgba(18, 47, 95, 0.34);
  box-shadow: 0 0 0 3px rgba(18, 47, 95, 0.08);
}
...
.location-field .input-wrap > .icon {
  color: var(--navy);
}
...
.location-pin-img.compact {
  width: 17px;
  height: 17px;
  flex-basis: 17px;
}

.role-field .input-wrap > .icon,
.input-wrap > .icon {
  color: #585b63;
}
...
.employee-modal .form-two {
  gap: 28px 42px;
}

.employee-modal .input-wrap {
  min-height: 44px;
  border-radius: 8px;
}
